...
public final class DBNinja {
    ...
    private static String user = "AntnPzDB_27hk";
    //enter your password here
    private static String password = "we_love_kevin";
    //enter your database name here
    private static String database_name = "AntonPizzaDB_mu3m";
    ...
    public static void addOrder(Order o) throws SQLException, IOException
    {
		...
		*/
        ...
        Iterator<Pizza> pizzaIterator = o.getPizzas().iterator();
        while (pizzaIterator.hasNext()) {
            Pizza cur = pizzaIterator.next();
            addPizza(cur, orderID);
        }

        Iterator<Discount> discountIterator = o.getDiscounts().iterator();
        while (discountIterator.hasNext()) {
            Discount cur = discountIterator.next();
            addOrderDiscount(cur, orderID);
        }

        connect_to_db();
        switch (o.getType()) {
            case "pick-up":
                query = "INSERT INTO PICKUP_ORDER (Customer_ID, Order_ID) " + "VALUES (?, ?)";
                try {
                    PreparedStatement stmt = conn.prepareStatement(query);
                    stmt.clearParameters();
            ...
            case "dine-in":
                query = "INSERT INTO DINE_IN (customer_id, table_number, order_id)" + "VALUES (null,?,?)";
                try {

                    PreparedStatement stmt = conn.prepareStatement(query);
                    stmt.clearParameters();

                    IDineInCustomer dine_in = (DineInCustomer) o.getCustomer();
                    int table = dine_in.getTableNum();
                    stmt.setInt(1,table);   // setting to 1 for now
                    stmt.setInt(2, orderID);
                    stmt.execute();
                } catch(SQLException e) {
                ...
                String query1 = "INSERT INTO SEAT (Order_ID, Seat_num)" + "VALUES (?,?)";
                try{
                  PreparedStatement stm = conn.prepareStatement(query1);
                  stm.clearParameters();
                  // DineInCustomer dine_in = (DineInCustomer) o.getCustomer();
                  // List<Integer>seatnum = dine_in.getSeats();
                  stm.setInt(1,orderID);
                  stm.setInt(2, 1);//seatnum)
                }catch(SQLException e) {
                    System.out.println("Error adding dine in order");
                    while(e!=null) {
                        System.out.println("Message     : " + e.getMessage());
    ...
    private static void addOrderDiscount(Discount discount, int orderID) throws SQLException, IOException
    {
        connect_to_db();
        String query = "INSERT INTO DISCOUNT_ORDER VALUES (?,?)";
        try {
            PreparedStatement stmt = conn.prepareStatement(query);
            stmt.clearParameters();
            stmt.setInt(1, discount.getID());
            stmt.setInt(2, orderID);
            stmt.execute();
        } catch(SQLException e) {
            System.out.println("Error adding to discount order");
            while(e!=null) {
                System.out.println("Message     : " + e.getMessage());
                e = e.getNextException();
            }
    ...
    private static int getMaxOrderID() throws SQLException, IOException
    {
        int id = 0;
        connect_to_db();
        try {
            ...
            System.out.println("Error loading max order id");
            while(e!=null) {
                System.out.println("Message     : " + e.getMessage());
                e = e.getNextException();
            }
    ...
    {
        String query = "INSERT INTO PIZZA (Status, Price, Cost_to_comp, Order_ID, Base_Price_ID, Time_Stamp) VALUES (?,?,?,?,?,?)";
        int basePriceID = getBasePriceID(p.getSize(), p.getCrust());
        double costToCompany = getCostToComp(basePriceID, p.getToppings());
        connect_to_db();
        try {
            PreparedStatement stmt = conn.prepareStatement(query);
            stmt.clearParameters();
            stmt.setString(1, "Sent to Kitchen");
            stmt.setDouble(2, p.calcPrice());
            stmt.setDouble(3, costToCompany);
            stmt.setInt(4, orderID);
            stmt.setDouble(5, basePriceID);
        ...
        try {
            ...
            System.out.println("Error loading pizza id");
            while(e!=null) {
                System.out.println("Message     : " + e.getMessage());
                e = e.getNextException();
            }
        ...
        // reduce toppings and add to pizza
        double reduceBy = 1.0;
        Iterator<Topping> toppingInterator = p.getToppings().iterator();
        while (toppingInterator.hasNext()) {
            Topping cur = toppingInterator.next();
            reduceBy = getAmountOfToppingToUse(p.getSize(), cur.getName());
            if (cur.getExtra()) {
                addToppingToPizza(pizzaID, cur.getID(), "yes");
                adjustToppingAmounts(cur.getID(), reduceBy*2);
            } else {
                addToppingToPizza(pizzaID, cur.getID(), "no");
                adjustToppingAmounts(cur.getID(), reduceBy);
            }
        }

        // apply discounts
        Iterator<Discount> discountIterator = p.getDiscounts().iterator();
        while (discountIterator.hasNext()) {
            Discount cur = discountIterator.next();
            addPizzaDiscount(cur, pizzaID);
        }
        conn.close();
    }

    private static void addToppingToPizza(int pizzaID, int toppingID, String extra) throws SQLException, IOException
    {
        connect_to_db();
        String query = "INSERT INTO PIZZA_TOPPING (Topping_ID, Pizza_ID, Extra) VALUES (?,?,?);";
        try {
            PreparedStatement stmt = conn.prepareStatement(query);
            stmt.clearParameters();
            ...
            System.out.println("Error adding to topping to pizza");
            while(e!=null) {
                System.out.println("Message     : " + e.getMessage());
                e = e.getNextException();
            }
    ...
    private static double getAmountOfToppingToUse(String size, String name) throws SQLException, IOException
    {
        connect_to_db();
        double amount = 0.0;
        ...
        String query = "SELECT " + size + " FROM TOPPINGS WHERE Name = '" + name + "';";
        Statement stmt = conn.createStatement();
        try{
            ResultSet rset = stmt.executeQuery(query);
            while(rset.next())
            ...
            System.out.println("Error loading amount of topping to use");
            while (e != null) {
                System.out.println("Message     : " + e.getMessage());
                e = e.getNextException();
            }
    ...
    private static void adjustToppingAmounts(int id, double amount) throws SQLException, IOException
    {
       connect_to_db();
       try {
            String query = "UPDATE TOPPINGS SET Inventory = Inventory - ? WHERE ID = ?;";
            PreparedStatement stmt = conn.prepareStatement(query);
    ...
    private static void addPizzaDiscount(Discount discount, int pizzaID) throws SQLException, IOException
    {
        connect_to_db();
        String query = "INSERT INTO DISCOUNT_PIZZA VALUES (?,?)";
        try {
            PreparedStatement stmt = conn.prepareStatement(query);
            stmt.clearParameters();
            stmt.setInt(1, discount.getID());
            ...
            System.out.println("Error adding to discount pizza");
            while(e!=null) {
                System.out.println("Message     : " + e.getMessage());
                e = e.getNextException();
            }
    ...
    private static int getBasePriceID(String size, String crust) throws SQLException, IOException
    {
        ...
        int ID = 0;
        //add code to get the base price for that size and crust pizza Depending on how you store size and crust in your database, you may have to do a conversion
        String query = "SELECT ID, Price FROM BASE_PRICE WHERE Size = '" + size + "' AND Crust = '" + crust + "';";
        Statement stmt = conn.createStatement();
        try{
            ResultSet rset = stmt.executeQuery(query);
            while(rset.next())
            ...
            System.out.println("Error loading Base_price");
            while (e != null) {
                System.out.println("Message     : " + e.getMessage());
                e = e.getNextException();
            }
    ...
    private static double getCostToComp(int basePriceID, ArrayList<Topping> toppings) throws SQLException, IOException
    {
        double total = 0.0;
        Iterator<Topping> toppingInterator = toppings.iterator();
        while (toppingInterator.hasNext()) {
            Topping cur = toppingInterator.next();
            total += getToppingCost(cur.getID());
        }
        try {
            total+=getBaseCost(basePriceID);
        } catch(SQLException e) {
    ...
    private static double getBaseCost(int basePriceID) throws SQLException, IOException
    {
        double cost = 0.0;
        connect_to_db();
        try {
            Statement stmt = conn.createStatement();
            String query = "SELECT Cost FROM BASE_PRICE WHERE ID = " + basePriceID;
            ResultSet rset = stmt.executeQuery(query);
            while (rset.next()){
    ...
    private static double getToppingCost(int toppingID) throws SQLException, IOException
    {
        double cost = 0.0;
        connect_to_db();
        try {
            Statement stmt = conn.createStatement();
            String query = "SELECT Cost_Per_Unit FROM TOPPINGS WHERE ID = " + toppingID;
            ResultSet rset = stmt.executeQuery(query);
            while (rset.next()){
                cost = rset.getDouble(1);
            }
        } catch(SQLException e) {
            System.out.println("Error loading topping cost");
            while(e!=null) {
                System.out.println("Message     : " + e.getMessage());
                e = e.getNextException();
            }
    ...
    private static int getMaxCustomerID() throws SQLException, IOException
    {
      connect_to_db();
      int maxID = 0;
      ...
      try{
        ...
        while(rset.next())
        {
          ...
          System.out.println("Error loading MaxCustomerID");
          while(e!=null) {
              System.out.println("Message     : " + e.getMessage());
              e = e.getNextException();
          }
    ...
    public static void addCustomer(ICustomer c) throws SQLException, IOException
    {
		...
		*/
		//have to use for loops like in menu.java
        ...
        try{
            ...
            preparedStmt.setInt(1, getMaxCustomerID()+1);
            DeliveryCustomer deliver = (DeliveryCustomer) c;
            DineOutCustomer dine_out = (DineOutCustomer) c;
            preparedStmt.setString(2,dine_out.getPhone());
            preparedStmt.setString(3,dine_out.getName());
            preparedStmt.setString(4,deliver.getAddress());
    ...
    public static void CompleteOrder(Order o) throws SQLException, IOException
    {
        ...
        try {
            Statement stmt = conn.createStatement();
            String query = "UPDATE PIZZA SET Status = 'Completed by kitchen' WHERE Order_ID = " + o.getID() + ";";
    ...
    public static void AddToInventory(Topping t, double toAdd) throws SQLException, IOException
    {
        ...
        double inventory = t.getInv() + toAdd;
        System.out.println(inventory);
		try{
            Statement stmt = conn.createStatement();
            String query = "UPDATE TOPPINGS SET Inventory = " + inventory + "WHERE ID = "  + t.getID() + ";";
		    stmt.executeUpdate(query);
        }
        catch(SQLException e){
    ...
    */
    ...
    public static ArrayList<Order> getCurrentOrders() throws SQLException, IOException
    {
        ...
        //Find all order ids where the order is not completed
		String query = "SELECT Order_ID FROM PIZZA WHERE Status != 'Completed by kitchen'";
        //Find all order information
        //That will include a customer ID
        //string query1 = "SELECT ";
        //Then use that customer ID to get all the customer information from the relevant tables
        //Add the customer object to the order object
        //also need to select the ICustomer object and if it is pick_up, delivery, or dine_in
        Statement stmt = conn.createStatement();
        try{
            ResultSet rset = stmt.executeQuery(query);
            while(rset.next())
    ...
    public static ArrayList<Discount> getDiscountList() throws SQLException, IOException
    {
        ...
        try{
            ...
            while(rset.next())
            {
                ...
                double percent_discount = rset.getDouble(3);
                double Dollar_amt = rset.getDouble(4);
                discs.add(new Discount(Name, percent_discount/100, Dollar_amt, ID));
            }
        }
        catch(SQLException e){
    ...
    public static ArrayList<ICustomer> getCustomerList() throws SQLException, IOException
    {
        ...
        try{
            ...
            while(rset.next())
            {
                ...
                String Phone_number = rset.getString(2);
                String Address = rset.getString(4);
                custs.add(new DeliveryCustomer(ID, Name, Phone_number, Address));
...
/*
    ...
    private static Pizza getPizza(int ID)  throws SQLException, IOException
    {
        ...
        Pizza P;
        String query = "Select ID From PIZZA Where ID = " + ID +";";
        return P;
...
//
//        ICustomer C;
//        String query = "Select Name, Phone_number, Address From CUSTOMER where ID = " + ID + ";";
//        Statement stmt = conn.createStatement();
//        try{
//            ResultSet rset = stmt.executeQuery(query);
//            while(rset.next())
